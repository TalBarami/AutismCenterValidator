...
class Annotator:
    def __init__(self, root, annotations_filename, annotator_id):
        ...
        self.status_types = ['OK', 'Child detected as adult', 'Adult detected as child', 'Mixup', 'Missing skeleton of the child', 'Skip']
        # self.data_handler = DataHandler(osp.join(root, self.annotations_filename), osp.join(root, 'annotations'), osp.join(root, 'data'), self.annotator_id) TODO: Change
        ...
        while not df.empty:
            ...
            try:
                ...
                if not osp.exists(row['video_path']) or not osp.exists(row['data_path']):
                    ...
                    continue
                assert _idx == idx
                task = self.executor.submit(lambda: self.validate(opts=self.status_types))
